#!/usr/bin/env bash
# Phase 4 — build & deploy 4 MCP servers (account, transaction, ticket, evil-tools).
...
build_and_load evil-tools 1.0.0 "VARIANT=clean"

log_step "4.5 — build evil-tools v1.0.0-rugpull (mutated payload, same name+tag)"
build_and_load evil-tools 1.0.0-rugpull "VARIANT=rugpull"
...
kubectl_apply "$MANIFESTS_DIR/phase04-mcp-servers/evil-tools.yaml"
...
wait_for_ready deployment evil-tools      "$NS_BANK_EVIL"
