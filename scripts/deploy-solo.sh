...
#      what stops the lateral exfil from currency-converter to the C2 server).
...
# bank-vendors: only the agentgateway may proxy to currency-converter (so support-bot
# can still call convert_currency through the gateway). Pods inside
# bank-vendors cannot themselves reach into bank-mcp because their SAs
